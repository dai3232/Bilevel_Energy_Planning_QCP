...
runContext = rkkt.artifacts.create_run_context(projectRoot,'stage_0', ...
    'ModelContractVersion','1.0', ...
    'EffectiveConfigPath',fullfile(projectRoot,'config','stage_0.yaml'));
try
    result = rkkt.diagnostics.run_stage0_acceptance(runContext);
catch ME
...
try
    ...
    issues = rkkt.diagnostics.append_stage0_issue(issues,runContext,'S0-RUNNER', ...
...
try
    ...
    if isfield(manifest,'status') && strcmp(manifest.status,'RUNNING')
        rkkt.artifacts.finalize_run_manifest(runContext,'FAIL_RETRYABLE', ...
