...
5. `stage_0` 与 `stage_A1` 已通过，当前 `stage_A2` 为 `READY`；这只表示允许开展 A2，不表示 A2 已通过。

## 当前状态

- 已完成阶段：`stage_0 / PASS`、`stage_A1 / PASS`
- 当前阶段：`stage_A2 / READY`（尚未执行、尚未通过）
- A1 通过证据：`runs/20260720_131455_stage_A1_705f17da`；完整 KKT 为 471，小时块为 27、27、29，全局核心为 16，15 项阻断性验收全部通过。
- 当前算法验证路线：A1 已完成第 1 日第 8–10 小时“3 小时人工闭合测试窗（仅算法测试）”的单次方向等价验证；A2 将验证第 14 日完整 24 小时，随后验证第 14–20 日 7 天，最后才扩展到 365 天。
