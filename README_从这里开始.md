...
5. `stage_0`、`stage_A1` 与 `stage_A2` 已通过；原 `stage_A3` 数值验收仍为 PASS，但当前为证据修复而重新开放为 `READY`。`stage_A4` 已恢复为 `LOCKED`，尚未开始。

## 当前状态

- 已完成阶段：`stage_0 / PASS`、`stage_A1 / PASS`、`stage_A2 / PASS`
- 当前阶段：`stage_A3 / READY`（证据修复重新开放；原正式运行数值验收仍为 PASS）
- 下一阶段：`stage_A4 / LOCKED`（尚未开始、尚未执行）
...
- 原 A3 正式证据：`runs/20260721_080023_stage_A3_0fbd183e`；完整 KKT 为 18836，7 个日链为 589、590、589、590、590、590、590，既约系统为 4340，全局核心为 16，fixed_zero_map 为 422 项，6 项阻断性数值验收仍为 PASS。当前仅补齐审计证据和工程门禁，不篡改该历史运行。
- 当前算法验证路线：A1 已完成第 1 日第 8–10 小时“3 小时人工闭合测试窗（仅算法测试）”的单次方向等价验证；A2 已完成第 14 日完整 24 小时单次方向等价验证；A3 正在修复第 14–20 日证据导出与回归门禁；A4 尚未开始。
