...
4. 将 `交给Codex的首次任务.md` 作为第一次任务发送给 Codex。
5. 第一次只允许完成 `stage_0`。只有 `stage_0` 的阻断性验收全部通过，才允许进入 `stage_A1`。

## 当前状态

- 当前阶段：`stage_0`
- 当前状态：`READY`
- 当前算法验证窗口：先 1 日 3 小时，再 1 日 24 小时，再第 14-20 日 7 天，最后才扩展到 365 天。
