function evidence = run_stage_A2_tests(varargin)
%RUN_STAGE_A2_TESTS Run only the explicit Stage A2 fixed test suite.
% A1 regression is deliberately executed separately by main_stage_A2 into
% tests/a1_regression so the two inventories cannot be conflated.

repoRoot=fileparts(fileparts(mfilename('fullpath')));
addpath(genpath(fullfile(repoRoot,'src')));
parser=inputParser; parser.FunctionName=mfilename;
addParameter(parser,'EvidenceDirectory',"",@is_text_scalar);
addParameter(parser,'CommandText',"run_stage_A2_tests()",@is_text_scalar);
parse(parser,varargin{:});
evidenceDirectory=strip(string(parser.Results.EvidenceDirectory));
commandText=strip(string(parser.Results.CommandText));
assert(strlength(commandText)>0,'stageA2:tests:EmptyCommandText', ...
    'CommandText must be nonempty.');
[suite,inventory]=fixed_stage_a2_suite(repoRoot);
...
if ~allPass
    error('stageA2:tests:Failed', ...
        'One or more fixed Stage A2 tests failed or were incomplete.');
end
end

function [suite,inventory] = fixed_stage_a2_suite(repoRoot)
relativeFiles=[ ...
...
for fileIndex=1:numel(relativeFiles)
    ...
    assert(isfile(absolutePath),'stageA2:tests:MissingFixedTestFile', ...
        'Fixed A2 test file is missing: %s',absolutePath);
    fileSuite=matlab.unittest.TestSuite.fromFile(absolutePath);
    assert(~isempty(fileSuite),'stageA2:tests:EmptyFixedTestFile', ...
        'Fixed A2 test file defines no tests: %s',absolutePath);
...
assert(height(inventory)>0,'stageA2:tests:EmptyInventory', ...
    'The explicit A2 suite must contain tests.');
names=string(inventory.test_name);
assert(all(strlength(strip(names))>0) && numel(unique(names))==numel(names), ...
    'stageA2:tests:InvalidInventory', ...
    'A2 test names must be nonempty and unique.');
assert(all(startsWith(string(inventory.source_file),"tests/") & ...
    contains(string(inventory.source_file),"stage_a2")), ...
    'stageA2:tests:ForeignTest', ...
    'The fixed A2 inventory may contain only explicitly named A2 test files.');
end
...
timer=tic; diaryGuard=onCleanup(@close_diary_safely);
try
    diary(paths.console_log);
    fprintf('Fixed Stage A2 test inventory (%d tests):\n',height(inventory));
    ...
    fprintf(['Stage A2 summary: total=%d, passed=%d, failed=%d, ' ...
    ...
    wall=toc(timer);
    fprintf(2,'Stage A2 test/evidence failure:\n%s\n', ...
...
if isfile(directory), error('stageA2:tests:EvidencePathConflict', ...
        'Evidence directory conflicts with a file: %s',directory); end
if ~isfolder(directory)
    [created,message]=mkdir(directory);
    assert(created,'stageA2:tests:EvidenceDirectoryCreateFailed','%s',message);
end
targets=struct2cell(paths);
exists=cellfun(@(p)isfile(p)||isfolder(p),targets);
assert(~any(exists),'stageA2:tests:EvidenceArtifactExists', ...
    'Refusing to overwrite existing A2 evidence: %s', ...
...
assert(height(result)==height(inventory) && ...
    numel(unique(names))==height(inventory) && isequal(names,expected), ...
    'stageA2:tests:ResultInventoryMismatch', ...
    'A2 result identity/order differs from the persisted inventory.');
end
...
try
    if ~is_complete_junit(paths.results_xml,height(inventory))
        if isfile(paths.results_xml)
            ...
            assert(~isfile(backup),'stageA2:tests:JUnitBackupExists', ...
    ...
    fprintf(2,'Could not create A2 fallback JUnit: %s\n',persistenceException.message);
end
end
...
lines=["<?xml version=""1.0"" encoding=""UTF-8""?>"; ...
    sprintf('<testsuites name="stage_A2" tests="%d" failures="0" errors="%d" skipped="0" time="%.17g">',height(inventory),height(inventory),wall); ...
    sprintf('<testsuite name="fixed_stage_A2_suite" tests="%d" failures="0" errors="%d" skipped="0" time="%.17g">',height(inventory),height(inventory),wall)];
for k=1:height(inventory)
    lines=[lines;"<testcase classname=""stage_A2"" name="""+ ...
...
assert(fileId>=0,'stageA2:tests:EvidenceFileOpenFailed','%s',message);
guard=onCleanup(@()close_file_safely(fileId));
bytes=unicode2native(char(textValue),'UTF-8'); written=fwrite(fileId,bytes,'uint8');
assert(written==numel(bytes),'stageA2:tests:EvidenceFileWriteFailed', ...
    'Incomplete write for %s.',pathValue);
status=fclose(fileId); clear guard;
assert(status==0,'stageA2:tests:EvidenceFileCloseFailed', ...
