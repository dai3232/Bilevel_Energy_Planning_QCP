...
[paths,audit] = generate_stage_a4_reports( ...
...
[~,model] = validate_docx_package(paths.model_report);
...
verifyError(testCase,@()generate_stage_a4_reports( ...
...
verifyTrue(testCase,all(audit.status=="PASS"));
[~,details] = validate_docx_package(paths.run_summary);
verifyTrue(testCase,contains(details.document_text, ...
    "PENDING_REPORT_GATE"));
clear guard;
...
direction.direction_relative_error(2) = 1e-9;
write_table_csv_17g(directionPath,direction);
verifyError(testCase,@()generate_stage_a4_reports( ...
    testCase.TestData.context, ...
...
write_table_csv_17g(physicalPath,physical);
verifyError(testCase,@()generate_stage_a4_reports( ...
    testCase.TestData.context, ...
...
physical.status(1) = "FAIL";
write_table_csv_17g(physicalPath,physical);
write_json_file(testPath,struct( ...
...
[reportPaths,audit] = generate_stage_a4_reports( ...
    testCase.TestData.context, ...
...
[~,details] = validate_docx_package(reportPaths.run_summary);
...
paths = generate_stage_a4_reports( ...
    testCase.TestData.context,OutputDirectory=output);
verifyError(testCase,@()generate_stage_a4_reports( ...
    testCase.TestData.context,OutputDirectory=output), ...
    "stageA4:report:ArtifactExists");
incomplete = rmfield(paths,"run_summary");
verifyError(testCase,@()validate_stage_a4_report_set( ...
...
write_table_csv_17g(fullfile(root,"acceptance","physical_audit.csv"),physical);
...
iterations = table(iteration,state_revision,objective_scaled, ...
    ...
    convergence_passed);
write_table_csv_17g(fullfile(root,"iterations","iteration_summary.csv"), ...
    iterations);
...
write_table_csv_17g(fullfile(root,"iterations","direction_audit.csv"), ...
    direction);
...
write_table_csv_17g(fullfile(root,"results","capacity_results.csv"),capacity);

hourly = synthetic_hourly();
write_table_csv_17g(fullfile(root,"results", ...
...
write_table_csv_17g(fullfile(root,"matrices","matrix_manifest.csv"),matrices);
