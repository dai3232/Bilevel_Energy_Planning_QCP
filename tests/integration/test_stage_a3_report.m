...
config=rkkt.model.load_stage_a3_configuration(root); data=rkkt.data.load_project_data(root);
index=rkkt.indexing.build_stage_a3_index(data,"RunId","A3_REPORT_TEST");
verification=rkkt.diagnostics.run_stage_a3_direction_verification(data,index,config);
...
context=rkkt.artifacts.create_run_context(project,"stage_A3","RunId","A3_REPORT_RUN", ...
...
rkkt.artifacts.export_stage_a3_artifacts(context,data,index,config,verification.linearization, ...
    verification.direct,verification.recursive,verification.audit, ...
    CodeScan=scan,Physical=verification.physical);
acceptance=rkkt.diagnostics.evaluate_stage_a3_acceptance_facts(root,verification);
rkkt.artifacts.write_table_csv_17g(context.acceptance_results_path,acceptance);
rkkt.artifacts.write_table_csv_17g(context.issue_log_path,rkkt.diagnostics.new_stage_a3_issue_log());
    write_a3_test_evidence(context.tests_dir,root);
...
paths=rkkt.reporting.generate_stage_a3_reports(testCase.TestData.context, ...
...
[~,details]=rkkt.reporting.validate_docx_package(paths.model_report);
verifyTrue(testCase,contains(details.document_text,"18836"));
verifyTrue(testCase,contains(details.document_text,"4340"));
verifyTrue(testCase,contains(details.document_text,"递推约化排列审计"));
verifyTrue(testCase,contains(details.document_text,"正逆复合严格恢复恒等"));
...
function testDisplaysAllJsonDecodedEnabledComponents(testCase)
expected=["OBJ_INVEST";"CAP_BOUNDS_GLOBAL";"DURATION_GLOBAL"; ...
    "DAILY_CAPACITY_COPY";"CAPACITY_BINDING"; ...
    "HOURLY_POWER_BALANCE";"FORMAL_DAILY_SOC"];
manifest=jsondecode(fileread(testCase.TestData.context.run_manifest_path));
verifyTrue(testCase,iscell(manifest.enabled_components), ...
    'Fixture must exercise the jsondecode multi-element cell representation.');
verifyEqual(testCase,string(manifest.enabled_components(:)),expected);

output=fullfile(testCase.TestData.project,"reports_enabled_components");
paths=rkkt.reporting.generate_stage_a3_reports(testCase.TestData.context, ...
    'OutputDirectory',output,'FinalStatusCandidate','PASS');
[valid,details]=rkkt.reporting.validate_docx_package(paths.model_report);
verifyTrue(testCase,valid,strjoin(details.errors,"; "));
for k=1:numel(expected)
    verifyTrue(testCase,contains(details.document_text,expected(k)), ...
        "Missing enabled component in report: "+expected(k));
end
verifyTrue(testCase,contains(details.document_text, ...
    "Manifest启用组件="+strjoin(expected.',", ")+"。"));
end
...
bad=rkkt.diagnostics.set_stage_a3_acceptance_result(bad,"SA3-EQ-001","FAIL", ...
    "1","forced test failure","iterations/direction_comparison.csv");
rkkt.artifacts.write_table_csv_17g(pathValue,bad);
verifyError(testCase,@()rkkt.reporting.generate_stage_a3_reports(testCase.TestData.context, ...
...
backup=pathValue+".test_backup"; [moved,message]=movefile(pathValue,backup);
assert(moved,message); guard=onCleanup(@()restore_file(pathValue,backup));
issues=rkkt.diagnostics.new_stage_a3_issue_log();
issues=rkkt.diagnostics.append_stage_a3_issue(issues,testCase.TestData.context,"SA3-RUN", ...
    "test","object","symptom","message","cause","solution", ...
    "OPEN","run_manifest.json");
rkkt.artifacts.write_table_csv_17g(pathValue,issues);
verifyError(testCase,@()rkkt.reporting.generate_stage_a3_reports(testCase.TestData.context, ...
...
rkkt.reporting.generate_stage_a3_reports(testCase.TestData.context, ...
    'OutputDirectory',output,'FinalStatusCandidate','PASS');
verifyError(testCase,@()rkkt.reporting.generate_stage_a3_reports(testCase.TestData.context, ...
    'OutputDirectory',output,'FinalStatusCandidate','PASS'), ...
    "stageA3:report:ArtifactExists");
context=create_synthetic_context(testCase,"A3_FAILURE_REPORT_RUN");
acceptance=rkkt.diagnostics.initialize_stage_a3_acceptance(testCase.TestData.root);
for row=1:height(acceptance)
    acceptance=rkkt.diagnostics.set_stage_a3_acceptance_result(acceptance, ...
...
pathValue=rkkt.reporting.generate_stage_a3_failure_report(context,exception, ...
    rkkt.diagnostics.new_stage_a3_issue_log(),acceptance);
...
function testRejectsA3InventoryOutsideControlledContract(testCase)
pathValue=fullfile(testCase.TestData.context.tests_dir,"test_inventory.csv");
backup=pathValue+".test_backup"; [moved,message]=movefile(pathValue,backup);
assert(moved,message); guard=onCleanup(@()restore_file(pathValue,backup));
inventory=readtable(backup,'FileType','text','Delimiter',',', ...
    'ReadVariableNames',true, ...
    'TextType','string','VariableNamingRule','preserve');
rkkt.artifacts.write_table_csv_17g(pathValue,inventory(1:end-1,:));
verifyError(testCase,@()rkkt.reporting.generate_stage_a3_reports(testCase.TestData.context, ...
    'OutputDirectory',fullfile(testCase.TestData.project,"bad_inventory"), ...
    'FinalStatusCandidate','PASS'),"stageA3:tests:ExpectedInventoryMismatch");
clear guard;
end
...
context=rkkt.artifacts.create_run_context(testCase.TestData.project,"stage_A3", ...
...
function write_a3_test_evidence(directory,root)
expectedPath=fullfile(root,"tests","stage_A3_expected_test_inventory.csv");
inventory=readtable(expectedPath,'Delimiter',',','ReadVariableNames',true, ...
    'TextType','string','VariableNamingRule','preserve');
audit=rkkt.diagnostics.validate_stage_a3_test_inventory_contract(inventory,root);
count=height(inventory); names=string(inventory.test_name);
results=table(names,true(count,1),false(count,1),false(count,1), ...
    zeros(count,1),repmat("",count,1), ...
    'VariableNames',{'test_name','passed','failed','incomplete', ...
    'duration_seconds','details'});
...
rkkt.artifacts.write_json_file(fullfile(directory,"test_summary.json"),struct( ...
    "test_total",count,"test_passed",count,"test_failed",0, ...
    "test_incomplete",0,"duration_seconds",0, ...
    "expected_inventory_match",logical(audit.matches_expected), ...
    "expected_inventory_count",double(audit.expected_count), ...
    "expected_inventory_relative_path", ...
    char(audit.expected_inventory_relative_path), ...
    "expected_inventory_sha256",char(audit.expected_inventory_sha256), ...
    "test_names_unique",logical(audit.test_names_unique), ...
    "inventory_missing_count",double(audit.missing_count), ...
    "inventory_unexpected_count",double(audit.unexpected_count)));
end
