...
paths=generate_stage_a3_reports(testCase.TestData.context, ...
    'OutputDirectory',output,'FinalStatusCandidate','PASS');
[valid,details]=validate_docx_package(paths.model_report);
verifyTrue(testCase,valid,strjoin(details.errors,"; "));
...
write_table_csv_17g(pathValue,inventory(1:end-1,:));
verifyError(testCase,@()generate_stage_a3_reports(testCase.TestData.context, ...
...
inventory=readtable(expectedPath,'Delimiter',',','ReadVariableNames',true, ...
    'TextType','string','VariableNamingRule','preserve');
audit=validate_stage_a3_test_inventory_contract(inventory,root);
