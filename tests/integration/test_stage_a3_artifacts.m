...
index=rkkt.indexing.build_stage_a3_index(data,"RunId","A3_ARTIFACT_TEST");
...
context=rkkt.artifacts.create_run_context(temporaryProject,"stage_A3", ...
    "RunId","stage_A3_artifact_integration","ManifestMetadata",metadata);
exported=rkkt.artifacts.export_stage_a3_artifacts(context,data,index,config, ...
...
acceptance=rkkt.diagnostics.evaluate_stage_a3_acceptance_facts(testCase.TestData.root, ...
...
manifest=rkkt.artifacts.mark_stage_a3_solver_execution(context);
verifyTrue(testCase,manifest.a3_solver_executed);
manifest=rkkt.artifacts.record_stage_a3_test_command(context,"run_stage_A3_tests()", ...
    "run_stage_A2_tests()","run_stage_A1_tests()");
verifyEqual(testCase,string(manifest.actual_test_command.stage_a3), ...
    "run_stage_A3_tests()");
verifyError(testCase,@()rkkt.artifacts.mark_stage_a3_solver_execution(context), ...
    "stageA3:artifacts:SolverExecutionAlreadyMarked");
verifyError(testCase,@()rkkt.artifacts.record_stage_a3_test_command(context,"x","y","z"), ...
...
evidence=rkkt.artifacts.write_stage_a3_evidence_hashes(testCase.TestData.context);
verifyTrue(testCase,all(evidence.status=="PASS"));
verifyEqual(testCase,nnz(evidence.scope=="reports"),3);
verifyError(testCase,@()rkkt.artifacts.write_stage_a3_evidence_hashes( ...
...
rkkt.artifacts.export_stage_a3_artifacts(testCase.TestData.context,testCase.TestData.data, ...
