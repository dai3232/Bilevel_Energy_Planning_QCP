...
index = rkkt.indexing.build_stage_a4_index( ...
    data,config,"RunId","A4_RESULT_ARTIFACT_TEST");
...
for row = 1:height(hashes)
    ...
    verifyEqual(testCase,rkkt.data.compute_sha256_file(target), ...
...
invalidState.xi(end) = [];
verifyError(testCase,@()rkkt.artifacts.export_stage_a4_result_artifacts( ...
