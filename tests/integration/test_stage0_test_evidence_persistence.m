...
write_stage0_exception_junit(junitPath, inventory, pi, exception);
...
verifyError(testCase, @() write_stage0_exception_junit( ...
...
update_running_run_manifest(context,struct('test_total',14, ...
    'candidate_terminal_status','FAIL_RETRYABLE'));
finalManifest = finalize_run_manifest(context,'FAIL_RETRYABLE');
...
results = table(testNames,true(14,1),false(14,1),false(14,1), ...
    ...
    'duration_seconds','details'});
write_table_csv_17g(fullfile(testsDirectory,'test_inventory.csv'),inventory);
write_table_csv_17g(fullfile(testsDirectory,'test_results.csv'),results);
...
write_json_file(fullfile(testsDirectory,'test_summary.json'),struct( ...
...
write_table_csv_17g(environmentPath,environment);
...
write_table_csv_17g(fullfile(testsDirectory, ...
...
manifest = struct( ...
    ...
    'sha256',char(compute_sha256_file(string(environmentPath))), ...
    'all_checks_passed',true));
write_json_file(fullfile(runRoot,'run_manifest.json'),manifest);

context = struct('root',runRoot,'tests_dir',testsDirectory);
reportPath = generate_stage0_test_evidence_report(context);
[valid,details] = validate_docx_package(reportPath);
...
verifyError(testCase,@() generate_stage0_test_evidence_report(context), ...
...
result = run_stage0_test_evidence_addendum(context,commandText);
...
verifyEqual(testCase, ...
    compute_sha256_file(string(context.test_evidence_manifest_path)), ...
    string(manifest.test_evidence_manifest_sha256));
[reportValid,reportDetails] = validate_docx_package(result.report_path);
...
for k = 1:rowCount
    ...
    sha(k) = compute_sha256_file(string(filePath));
end
