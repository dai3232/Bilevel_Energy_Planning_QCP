...
index=rkkt.indexing.build_stage_a2_index(data,"RunId","A2_ARTIFACT_TEST");
...
context=rkkt.artifacts.create_run_context(temporaryProject,"stage_A2", ...
    "RunId","stage_A2_artifact_integration");
exported=rkkt.artifacts.export_stage_a2_artifacts(context,data,index,config, ...
...
manifest=rkkt.artifacts.mark_stage_a2_solver_execution(context);
verifyTrue(testCase,manifest.a2_solver_executed);
manifest=rkkt.artifacts.record_stage_a2_test_command(context,"run_stage_A2_tests()", ...
    "run_stage_A1_tests()");
...
verifyEqual(testCase,string(manifest.actual_test_command.stage_a1_regression), ...
    "run_stage_A1_tests()");
verifyError(testCase,@()rkkt.artifacts.mark_stage_a2_solver_execution(context), ...
    "stageA2:artifacts:SolverExecutionAlreadyMarked");
verifyError(testCase,@()rkkt.artifacts.record_stage_a2_test_command(context,"x","y"), ...
...
evidence=rkkt.artifacts.write_stage_a2_evidence_hashes(testCase.TestData.context);
verifyTrue(testCase,all(evidence.status=="PASS"));
verifyEqual(testCase,nnz(evidence.scope=="reports"),3);
verifyError(testCase,@()rkkt.artifacts.write_stage_a2_evidence_hashes( ...
...
rkkt.artifacts.export_stage_a2_artifacts(testCase.TestData.context,testCase.TestData.data, ...
