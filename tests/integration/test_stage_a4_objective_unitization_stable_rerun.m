...
% The user-approved A4 threshold revision is the current protected
% governance baseline; the before/after comparison above still forbids
% mutation during the R1 replay.
...
verify_hash(testCase,hashes,"config/solver.yaml", ...
    "5a1f86bb6dc51ffec6f3da7690eeb3e3f6e23191270bdbbf0439e9ccfb10edbf");
...
verifyEqual(testCase,rkkt.data.compute_sha256_file(pathValue), ...
...
expected = [ ...
    ...
    "14c5501a17fc317a5653b8d55ac29f6b103ddcfb3004aef807acff6bde4e62b3"];
for k = 1:numel(paths)
    verifyEqual(testCase,rkkt.data.compute_sha256_file(fullfile(root, ...
...
root = testCase.TestData.project_root;
result = testCase.TestData.result;
exporter = fullfile(root,"src","+rkkt","+diagnostics", ...
...
root = testCase.TestData.project_root;
result = testCase.TestData.result;
[r1Scan,dependencyClosure] = rkkt.diagnostics.scan_stage_a4_r1_forbidden_code(root);
...
requiredDependencies = [ ...
    ...
    "src/+rkkt/+diagnostics/run_stage_a4_objective_unitization_r1_diagnostic.m"
    "src/+rkkt/+diagnostics/audit_stage_a4_r1_forbidden_execution.m"
    "src/+rkkt/+diagnostics/make_stage_a4_r1_blocking_audit.m"
    "src/+rkkt/+diagnostics/scan_stage_a4_r1_forbidden_code.m"
    "src/+rkkt/+diagnostics/run_stage_a4_five_iteration_diagnostic.m"
    ...
    "src/+rkkt/+solver/solve_stage_a_multiday_full_kkt_direction.m"
    "src/+rkkt/+solver/update_primal_dual_state.m"];
...
noFallbackBlocking = rkkt.diagnostics.make_stage_a4_r1_blocking_audit( ...
    result.unscaled_five,result.scaled_five,result.scaled_twenty, ...
...
sharedScan = rkkt.diagnostics.scan_stage_a4_forbidden_code(root,config);
...
for k = 1:numel(relative)
    sha256(k) = rkkt.data.compute_sha256_file(fullfile(projectRoot, ...
