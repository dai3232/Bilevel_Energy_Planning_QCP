...
result = run_stage_a4_iteration24_scaling_stress(root);
...
verifyEqual(testCase,r.authority_manifest_sha256, ...
    compute_sha256_file(fullfile(testCase.TestData.root,"runs", ...
...
verifyEqual(testCase,r.effective_config.sha256, ...
    compute_sha256_file(fullfile(testCase.TestData.root,"config", ...
...
relative = [ ...
    "src/solver/private/equilibrate_symmetric_congruence.m"; ...
...
verifyTrue(testCase,contains(string(fileread(fullfile(root, ...
    "src/diagnostics/scan_stage_a4_iteration24_scaling_stress_forbidden_execution.m"))), ...
...
export_stage_a4_iteration24_scaling_stress(context,r, ...
...
write_table_csv_17g_atomic(fullfile(temporaryRoot,"input_hashes.csv"), ...
    r.input_hashes);
...
write_json_file(fullfile(temporaryRoot,"historical_runs_audit.json"), ...
