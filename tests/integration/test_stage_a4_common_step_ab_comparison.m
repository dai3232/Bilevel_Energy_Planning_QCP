...
auditSource = string(fileread(fullfile(testCase.TestData.project_root, ...
    "src","+rkkt","+diagnostics","audit_stage_a4_complementarity_change.m")));
...
data = rkkt.data.load_project_data(testCase.TestData.project_root);
index = rkkt.indexing.build_stage_a4_index( ...
    data,result.config,"RunId","A4_2C_SCOPE_GUARD_TEST");
state = rkkt.model.initialize_stage_a4_state(data,index,result.config);
verifyError(testCase,@()rkkt.ipm.execute_stage_a4_iteration( ...
...
verifyError(testCase,@()rkkt.ipm.execute_stage_a4_iteration( ...
    state,data,index,result.config,"StepStrategy","common_min"), ...
    "stageA4:iteration:ExperimentalCommonStepScope");
verifyError(testCase,@()rkkt.diagnostics.run_stage_a4_five_iteration_diagnostic( ...
