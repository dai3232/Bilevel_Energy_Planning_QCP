...
config = rkkt.model.load_stage_a1_configuration(repositoryRoot);
data = rkkt.data.load_project_data(repositoryRoot);
index = rkkt.indexing.build_stage_a1_index(data,"RunId","A1_ARTIFACT_TEST");
...
context = rkkt.artifacts.create_run_context(temporaryProject,"stage_A1", ...
...
exported = rkkt.artifacts.export_stage_a1_artifacts(context,data,index,config, ...
...
stage0 = rkkt.artifacts.create_run_context(testCase.TestData.temporaryProject,"stage_0", ...
...
context = rkkt.artifacts.create_run_context(testCase.TestData.temporaryProject,"stage_A1", ...
    "RunId","stage_A1_execution_marker","ManifestMetadata",metadata);
manifest = rkkt.artifacts.mark_stage_a1_solver_execution(context);
verifyTrue(testCase,logical(manifest.a1_solver_executed));
verifyTrue(testCase,isfield(manifest,"solver_execution_started_at"));
manifest = rkkt.artifacts.record_stage_a1_test_command(context, ...
    "run_stage_A1_tests('EvidenceDirectory','temporary/tests')");
verifyEqual(testCase,string(manifest.actual_test_command), ...
    "run_stage_A1_tests('EvidenceDirectory','temporary/tests')");
verifyError(testCase,@() rkkt.artifacts.record_stage_a1_test_command(context,"duplicate"), ...
    "stageA1:artifacts:TestCommandAlreadyRecorded");
verifyError(testCase,@() rkkt.artifacts.mark_stage_a1_solver_execution(context), ...
...
for row = 1:height(hashes)
    ...
    verifyEqual(testCase,lower(string(hashes.sha256(row))), ...
        rkkt.data.compute_sha256_file(string(filePath)));
    info = dir(filePath);
...
evidence = rkkt.artifacts.write_stage_a1_evidence_hashes(testCase.TestData.context,reports);
...
verifyError(testCase,@() rkkt.artifacts.write_stage_a1_evidence_hashes( ...
...
fresh = rkkt.artifacts.create_run_context(testCase.TestData.temporaryProject,"stage_A1", ...
...
rkkt.artifacts.export_stage_a1_artifacts(context,testCase.TestData.data, ...
