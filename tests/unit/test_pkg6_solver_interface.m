...
linearizationArtifact = rkkt.validation.packageArtifact("model", ...
    "统一线性化模块输出.mat");
reducedArtifact = rkkt.validation.packageArtifact( ...
    "solver","既约KKT模块输出.mat");
...
productionFile = fullfile(testCase.TestData.repositoryRoot, ...
    "src","+rkkt","+solver","form_stage_a_multiday_day_response.m");
...
verifyNotEmpty(testCase,regexp(validationSources(7), ...
    'modules\.fullKKT\s*=\s*rkkt\.validation\.loadResult\s*\(',"once"));
verifyNotEmpty(testCase,regexp(validationSources(7), ...
