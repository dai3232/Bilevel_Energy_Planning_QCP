...
verifyEqual(testCase,config.a4_2c_iteration_count,5);
verifyEqual(testCase,config.a4_2c_run_purpose, ...
    "five_iteration_independent_vs_common_step_controlled_ab_comparison");
verifyEqual(testCase,config.a4_2c_baseline_strategy, ...
    "independent_primal_dual_steps");
verifyEqual(testCase,config.a4_2c_experimental_strategy, ...
    "common_minimum_step");
...
verifyEqual(testCase,config.initialization.global_capacity,"midpoint");
verifyTrue(testCase, ...
...
verifyEqual(testCase, ...
    ...
    "AbsTol",0);
verifyEqual(testCase,config.initialization.equality_multipliers,0, ...
    "AbsTol",0);
verifyEqual(testCase,config.initialization.inequality_multiplier,1, ...
    "AbsTol",0);
...
verifyEqual(testCase,config.convergence.primal_equality_inf,1e-6, ...
    "AbsTol",0);
verifyEqual(testCase,config.convergence.primal_inequality_inf,1e-6, ...
    "AbsTol",0);
verifyEqual(testCase,config.convergence.dual_inf,1e-6,"AbsTol",0);
verifyEqual(testCase, ...
    config.convergence.normalized_complementarity_gap,1e-6,"AbsTol",0);
verifyEqual(testCase,config.tolerances.direction_relative_2norm,1e-6, ...
    "AbsTol",0);
verifyEqual(testCase, ...
    config.tolerances.recursive_full_kkt_relative_residual,1e-6, ...
    "AbsTol",0);
verifyEqual(testCase,config.direction_relative_tolerance,1e-10, ...
    "AbsTol",0);
verifyEqual(testCase,config.recursive_full_kkt_residual_tolerance,1e-10, ...
    "AbsTol",0);
