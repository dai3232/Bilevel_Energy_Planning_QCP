...
step = rkkt.solver.compute_fraction_to_boundary_step([2;4;3],[-1;-8;1],0.9);
...
value = [1e-12;1];
direction = [-1;0];
step = rkkt.solver.compute_fraction_to_boundary_step(value,direction,0.9995);
...
updated = rkkt.solver.update_primal_dual_state(state,direction,0.25,0.5);
...
verifyError(testCase, ...
    @()rkkt.solver.compute_fraction_to_boundary_step([1;0],[0;0],0.9995), ...
    "stageA4:step:NonpositiveValues");
verifyError(testCase, ...
    @()rkkt.solver.compute_fraction_to_boundary_step([1;2],[0;NaN],0.9995), ...
    "stageA4:step:NonfiniteVector");
verifyError(testCase, ...
    @()rkkt.solver.compute_fraction_to_boundary_step([1;2],[0;1;2],0.9995), ...
    "stageA4:step:DimensionMismatch");
verifyError(testCase, ...
    @()rkkt.solver.compute_fraction_to_boundary_step([1,2],[0;1],0.9995), ...
    "stageA4:step:InvalidVector");
verifyError(testCase, ...
    @()rkkt.solver.compute_fraction_to_boundary_step([1;2],[0;1],1), ...
...
verifyError(testCase, ...
    @()rkkt.solver.update_primal_dual_state(state,direction,0,0.5), ...
    "stageA4:update:InvalidStep");
badState = state;
badState.l(1) = 0;
verifyError(testCase, ...
    @()rkkt.solver.update_primal_dual_state(badState,direction,0.25,0.5), ...
    "stageA4:update:NonpositiveInput");
badDirection = direction;
badDirection.xi = 0;
verifyError(testCase, ...
    @()rkkt.solver.update_primal_dual_state(state,badDirection,0.25,0.5), ...
    "stageA4:update:DimensionMismatch");
verifyError(testCase, ...
    @()rkkt.solver.update_primal_dual_state(state,direction,0.5,0.5), ...
