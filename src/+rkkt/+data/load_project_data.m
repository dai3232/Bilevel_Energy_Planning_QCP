...
[hashes, hashesMatch] = rkkt.data.verify_input_hashes(projectRoot);
if ~hashesMatch
...
generalLocation = rkkt.data.locate_labeled_table(rawCells, sheetName, "基本参数", ...
...
countLocation = rkkt.data.locate_labeled_table(rawCells, sheetName, "基本参数", ...
...
weightLocation = rkkt.data.locate_labeled_table(rawCells, sheetName, "基本参数", ...
...
thermalCostLocation = rkkt.data.locate_labeled_table(rawCells, sheetName, "投资及度电成本参数", ...
...
hydroCostLocation = rkkt.data.locate_labeled_table(rawCells, sheetName, "投资及度电成本参数", ...
...
windHeaders(1) = "风电序号";
windLocation = rkkt.data.locate_labeled_table(rawCells, sheetName, "投资及度电成本参数", ...
...
solarHeaders(1) = "光伏序号";
solarLocation = rkkt.data.locate_labeled_table(rawCells, sheetName, "投资及度电成本参数", ...
...
storageHeaders = ["储能序号", "单位功率投资成本(元/MW)", ...
    ...
    "充电效率", "放电效率", "储能时长(hour)"];
storageLocation = rkkt.data.locate_labeled_table(rawCells, sheetName, "投资及度电成本参数", ...
...
energyBaseLocation = rkkt.data.locate_labeled_table(rawCells, sheetName, "投资及度电成本参数", ...
...
thermalLocation = rkkt.data.locate_labeled_table(rawCells, sheetName, "已知机组参数", ...
...
hydroLocation = rkkt.data.locate_labeled_table(rawCells, sheetName, "已知机组参数", ...
...
constraintHeaders = ["新能源总弃电率上限", "新能源总弃电率下限", ...
    ...
    "单时段最大计划总出力(MW)"];
constraintLocation = rkkt.data.locate_labeled_table(rawCells, sheetName, "约束参数", ...
...
timeLocation = rkkt.data.locate_labeled_table(rawCells, sheetName, "基本参数", timeHeaders, "same-row");
...
for unitIndex = 1:nHydro
    ...
    location = rkkt.data.locate_labeled_table(rawCells, sheetName, sectionLabel, hydroHeaders, "same-row");
...
for unitIndex = 1:nWind
    ...
    location = rkkt.data.locate_labeled_table(rawCells, sheetName, sectionLabel, availabilityHeaders, "same-row");
...
planLocation = rkkt.data.locate_labeled_table(rawCells, sheetName, "能源基地", planHeaders, "same-row");
