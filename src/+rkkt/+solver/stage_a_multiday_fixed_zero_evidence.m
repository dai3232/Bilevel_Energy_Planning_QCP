...
function evidence = stage_a_multiday_fixed_zero_evidence(index)
%STAGE_A_MULTIDAY_FIXED_ZERO_EVIDENCE Validate removed-variable recovery.
...
if istable(map)
    ...
    assert(count==0 || all(ismember( ...
        ...
        "stageAMultiday:solver:FixedZeroColumns", ...
...
elseif isstruct(map) && count>0
    assert(isfield(map,"fixed_value") && ...
        ...
        "stageAMultiday:solver:FixedZeroFields", ...
...
assert(numel(value)==count && numel(direction)==count && ...
    ...
    "stageAMultiday:solver:FixedZeroNonzero", ...
