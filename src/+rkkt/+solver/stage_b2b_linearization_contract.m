...
% configured daily water inequalities after the Stage-A inequality prefix.
...
if isfield(lin,"milestone_id")
    assert(ismember(string(lin.milestone_id),["B-2B","B2B","B-2C"]), ...
        "stageB2B:contract:Milestone", ...
        "Stage-B bordered linearization must identify B-2B or B-2C.");
end
...
nWaterInequality = height(waterIndex);
assert(nx==index.counts.variables && neq==index.counts.equalities && ...
    ni==index.counts.inequalities && ...
    nWaterInequality==2*numel(index.scope.days)*4 && ...
    nBaseInequality+nWaterInequality==ni, ...
    "stageB2B:contract:WaterCount", ...
    "The B-2B inequality prefix or daily-water suffix is inconsistent.");
waterRows = (nBaseInequality+(1:nWaterInequality)).';
assert(isequal(waterIndex.inequality_position(:),waterRows), ...
    "stageB2B:contract:WaterPositions", ...
    "Water inequalities must occupy the canonical suffix rows.");

days = read_scope_vector(lin.layout,"days",index.scope.days);
hours = read_scope_vector(lin.layout,"hours",1:24);
assert(isequal(days(:).',reshape(index.scope.days,1,[])) && ...
    isequal(hours(:).',1:24), ...
    "stageB2B:contract:Scope", ...
    "B-2B requires the configured complete-day scope and hours 1:24.");
if isfield(lin,"config")
    assert(nx==lin.config.expected_stage_a_primal_dimension && ...
        neq==lin.config.expected_stage_a_equality_dimension && ...
        ni==lin.config.expected_stage_a_inequality_dimension+ ...
            lin.config.expected_water_inequality_count && ...
        nBaseInequality==lin.config.expected_stage_a_inequality_dimension && ...
        nWaterInequality==lin.config.expected_water_inequality_count, ...
        "stageB2B:contract:ConfiguredDimensions", ...
        "The linearization dimensions disagree with the active B-2C profile.");
end
...
contract.neq = neq;
contract.nineq = ni;
contract.n_base_inequality = nBaseInequality;
contract.n_water_inequality = nWaterInequality;
contract.days = days(:).';
contract.hours = hours(:).';
contract.n_days = numel(days);
contract.n_hours = numel(hours);
contract.base_inequality = (1:nBaseInequality).';
