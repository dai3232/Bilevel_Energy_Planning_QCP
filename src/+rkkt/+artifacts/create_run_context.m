...
%
% context = rkkt.artifacts.create_run_context(projectRoot, stageId) creates a unique
...
% recorded as NOT_AVAILABLE rather than preventing run creation.
    ...
    gitInfo = rkkt.artifacts.capture_git_state(projectRoot);
    ...
    rkkt.artifacts.write_text_utf8(context.git_state_path, gitInfo.state_text);
    ...
    rkkt.artifacts.initialize_run_manifest(context, ...
...
function materialize_effective_config(context, configuredPath)
    ...
    elseif isExplicit
        ...
        placeholder = sprintf([ ...
            'stage_id: "%s"\r\n', ...
            'source: "NOT_PROVIDED"\r\n'], context.stage_id);
        rkkt.artifacts.write_text_utf8(context.effective_config_path, placeholder);
    end
end
