...
stateFingerprint = string(rkkt.artifacts.compute_stage_a4_checkpoint_state_fingerprint(state));
...
index = rkkt.indexing.build_stage_a4_index(data,config,"RunId", ...
...
reduced = rkkt.solver.eliminate_stage_a_multiday_inequality_directions(linearization);
...
if options.PerformFullKktAudit
    audit = rkkt.solver.solve_stage_a_multiday_full_kkt_direction(linearization);
...
try
    rkkt.solver.solve_stage_a_multiday_recursive_direction( ...
...
for k=1:numel(entries)
    ...
    actual="";
    if isfile(pathValue), actual=string(rkkt.data.compute_sha256_file(pathValue)); end
