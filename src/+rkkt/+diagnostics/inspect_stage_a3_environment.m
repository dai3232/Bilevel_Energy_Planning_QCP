function environment = inspect_stage_a3_environment()
%INSPECT_STAGE_A3_ENVIRONMENT Probe the serial Stage A3 MATLAB environment.

rows=repmat(empty_row(),0,1); checkedAt=now_text();
...
parallelInfo=ver('parallel'); installed=~isempty(parallelInfo);
if installed
...
rows(end+1)=make_row("PCT_INSTALLED","Parallel Computing Toolbox",false, ...
    "informational for serial A3",actual,installed,checkedAt, ...
    "metadata only; no pool is opened");
try
    available=logical(license('test','Distrib_Computing_Toolbox'));
    actual=sprintf("license_test=%d",available); details="license metadata only";
catch exception
...
rows(end+1)=make_row("PCT_LICENSE","Parallel Computing Toolbox 许可证", ...
    false,"informational for serial A3",actual,available,checkedAt,details);
rows(end+1)=make_row("PARALLEL_EXECUTION","A3 并行执行",true, ...
    "not executed","not executed",true,checkedAt, ...
    "A3 forms seven pure day responses serially");
