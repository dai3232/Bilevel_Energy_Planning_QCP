function write_stage_b2a_docx(outputPath,titleText,subtitleText,metadata,blocks)
%WRITE_STAGE_B2A_DOCX Create the B-2A Chinese report package.
% Uses the approved standard_business_brief/memo_masthead layout engine in
% a temporary package, then sanitizes stale Stage-A identity tokens.
...
for k = 1:numel(xmlFiles)
    ...
    textValue = replace(textValue, ...
        ...
        ["stage_B_2A","Stage B-2A","STAGE B-2A"]);
...
assert(moved,"stageB2A:report:PackageWrite","%s",message);
...
assert(fileId>=0,"stageB2A:report:Write","%s",message);
guard = onCleanup(@()close_file(fileId));
bytes = unicode2native(char(textValue),"UTF-8");
assert(fwrite(fileId,bytes,"uint8")==numel(bytes), ...
    "stageB2A:report:Write","Incomplete XML write: %s",pathValue);
assert(fclose(fileId)==0,"stageB2A:report:Write", ...
