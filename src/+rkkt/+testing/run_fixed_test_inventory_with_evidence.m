...
if strlength(directory)==0
    ...
    rkkt.artifacts.write_table_csv_17g(paths.results_csv, ...
        incomplete_results(inventory,"execution not completed"));
    rkkt.artifacts.write_json_file(paths.summary, ...
    ...
    try
        ...
        summary = make_summary(results,wall,"COMPLETE","","", ...
            expectedInventoryPath,options.SuiteLabel);
        rkkt.artifacts.write_table_csv_17g(paths.results_csv,results);
...
evidence = struct("inventory",inventory,"results",results, ...
    ...
    "expected_inventory_sha256", ...
        rkkt.data.compute_sha256_file(expectedInventoryPath));
if ~allPass
...
value = struct( ...
    ...
    "expected_inventory_sha256", ...
        char(rkkt.data.compute_sha256_file(expectedPath)), ...
...
rkkt.artifacts.write_table_csv_17g_atomic(paths.results_csv,results);
rkkt.artifacts.write_json_file(paths.summary,make_summary(results,wall,"ERROR", ...
...
rkkt.artifacts.write_table_csv_17g(paths.hashes,table(names,sha256,bytes,status));
end
