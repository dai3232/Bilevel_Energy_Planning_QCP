...
% fixed-zero prefixes are preserved exactly.  B-2A appends the configured
% daily water inequalities
...
assert(k==rowCount,"stageB2A:index:WaterRowCount", ...
    "The explicit water-row loop did not create %d rows.",rowCount);
...
assert(data.meta.nHydro==4 && data.meta.nHours==24 && ...
    ismember(config.time_scope_type, ...
        ["formal_7_day_scope","experimental_30_day_scope", ...
        "supplemental_365_day_serial_scope", ...
        "configured_contiguous_day_scope", ...
        "configured_explicit_day_scope"]) && ...
    ~isempty(config.days) && numel(unique(config.days))==numel(config.days) && ...
    all(config.days>=1) && all(config.days<=data.meta.nDays) && ...
    isequal(config.hours,1:24) && ...
    isequal(config.hydro_ids,1:4), ...
    "stageB2A:index:Scope", ...
    "B-2A index scope must use complete formal days and hydros 1:4.");
...
assert(height(water)==config.expected_water_inequality_count && ...
    height(water)==rowCount && ...
    isequal(water.row_position,(1:rowCount).') && ...
    isequal(water.day,repelem(config.days(:),8,1)) && ...
    isequal(water.hydro_id,repmat( ...
        repelem(config.hydro_ids(:),2,1),numel(config.days),1)) && ...
    isequal(water.bound_type,repmat( ...
        ["upper";"lower"],numel(config.days)*numel(config.hydro_ids),1)) && ...
    all(water.touched_hour_count==24) && ...
    numel(unique(water.constraint_id))==rowCount && ...
    numel(unique(water.global_row))==rowCount && ...
    index.counts.full_kkt_dimension==config.expected_full_kkt_dimension, ...
    "stageB2A:index:WaterInventory", ...
    "The canonical daily-water inventory is invalid.");
