...
entryPath = fullfile(projectRoot,"main_stage_A4_3.m");
runnerPath = fullfile(projectRoot,"src","solver", ...
...
filesScanned(row) = numel(closure);
details(row) = "dependency closure rooted at main_stage_A4_3.m";
if matchCount(row)>0
    status(row) = "FAIL";
end
...
for k = 1:n
    ...
    sha256(k) = compute_sha256_file(canonical);
    info = dir(canonical);
    bytes(k) = info.bytes;
