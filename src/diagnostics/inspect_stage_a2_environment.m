function environment = inspect_stage_a2_environment()
%INSPECT_STAGE_A2_ENVIRONMENT Probe the serial A2 MATLAB environment.

rows = repmat(empty_row(),0,1);
checkedAt = now_text();
releaseName = string(version('-release'));
rows(end+1) = make_row("MATLAB_RELEASE","MATLAB",true,"R2024a", ...
    sprintf("%s (%s)",string(version),releaseName),releaseName=="2024a", ...
    checkedAt,sprintf("arch=%s",computer('arch')));

diagonal = 4*ones(5,1); off = -ones(5,1);
matrix = spdiags([off,diagonal,off],[-1,0,1],5,5);
rhs = [(1:5)',eye(5,2)];
try
    solution = matrix\rhs;
    ...
    rows(end+1) = make_row("SPARSE_MLDIVIDE", ...
        ...
        issparse(matrix) && all(isfinite(solution),'all') && residual<=1e-12, ...
        checkedAt,"deterministic sparse multiple-RHS system");
catch exception
    rows(end+1) = make_row("SPARSE_MLDIVIDE", ...
        "稀疏多右端线性求解",true,"available","error",false,checkedAt, ...
        string(exception.identifier)+": "+string(exception.message));
end

try
    [L,D,p] = ldl(matrix,'vector');
    residual = norm(matrix(p,p)-L*D*L','fro')/max(1,norm(matrix,'fro'));
...
rows(end+1) = make_row("PCT_INSTALLED","Parallel Computing Toolbox",false, ...
    "informational for serial A2",actual,installed,checkedAt, ...
...
try
    ...
    available = false; actual = "license test error";
...
rows(end+1) = make_row("PCT_LICENSE", ...
    ...
    "informational for serial A2",actual,available,checkedAt,details);
rows(end+1) = make_row("PARALLEL_EXECUTION","A2 并行执行",true, ...
    "not executed","not executed",true,checkedAt, ...
    "A2 performs one serial Newton-direction comparison");
...
row.blocking=logical(blocking); row.expected=char(expected); row.actual=char(actual);
row.available=logical(available); row.checked_at=char(checkedAt);
row.details=char(details);
if row.available, row.status='PASS'; elseif ~row.blocking, row.status='NOT_APPLICABLE'; end
end
