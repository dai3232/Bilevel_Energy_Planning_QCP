...
inputException = [];
try
    [inputHashes,inputHashesPass] = verify_input_hashes(runContext.project_root);
    write_table_csv_17g(runContext.input_hashes_csv_path, ...
...
write_table_csv_17g(runContext.test_evidence_manifest_path,evidenceManifest);
evidenceManifestSha = compute_sha256_file( ...
...
update_running_run_manifest(runContext,updates);
...
reportException = [];
try
    generatedPath = generate_stage0_test_evidence_report(runContext);
    ...
    [reportValid,validation] = validate_docx_package(reportPath);
    if ~reportValid
        error('stage0:addendum:ReportValidationFailed','%s', ...
            strjoin(string(validation.errors),'; '));
    end
    reportSha = compute_sha256_file(string(reportPath));
catch exception
...
evidenceStable = evidenceFilesStillPresent && ...
    ...
    compute_sha256_file(string(runContext.test_evidence_manifest_path)) == ...
    evidenceManifestSha;
...
manifest = finalize_run_manifest(runContext,char(candidateStatus),updates);
...
for k = 1:n
    ...
    if existsFlag(k)
        ...
        sha(k) = compute_sha256_file(string(pathValue));
    end
end
...
if isfile(filePath)
    evidence.sha256 = char(compute_sha256_file(string(filePath)));
end
end
