...
%PARTITION_STAGE_A3_RECURSIVE_SYSTEM Build seven independent day chains.
...
assert(isequal(reduced.linearization_identity,contract.identity), ...
    ...
    "A3 partition and inequality elimination identities differ.");
assert(isequal(size(reduced.W),[contract.nx,contract.nx]) && ...
    numel(reduced.b_xi)==contract.nx, ...
    "stageA3:solver:ReducedSystemShape", ...
    "A3 reduced W or RHS has an invalid shape.");
...
if crossDayCouplingNnz~=0
    error("stageA3:solver:CrossDaySocCoupling", ...
        "A3 equality Jacobian contains %d cross-day hourly coefficient(s).", ...
        crossDayCouplingNnz);
end
...
if matrixRelative>options.AssemblyTolerance || rhsRelative>options.AssemblyTolerance
    error("stageA3:solver:RecursiveAssemblyMismatch", ...
...
for row = 1:n
    ...
    if ~isEquality(row)
        ...
        assert(height(metadata)==1, ...
            "stageA3:solver:PermutationVariableMetadata", ...
        ...
        assert(height(metadata)==1, ...
            "stageA3:solver:PermutationEqualityMetadata", ...
    ...
    if recursiveIndex(row)<=16
        ...
        assert(~isempty(dayPosition), ...
            "stageA3:solver:PermutationObjectRange", ...
...
assert(height(map)==n && numel(unique(map.recursive_solver_index))==n && ...
    ...
    "stageA3:solver:PermutationEvidence", ...
    "A3 recursive permutation evidence is not a nonidentity bijection.");
end
...
assert(isequal(size(R),[2,14]) && nnz(R-expected)==0, ...
    "stageA3:solver:DurationJacobian", ...
...
assert(nnz(lin.A(contract.y_duration,other))==0, ...
    "stageA3:solver:DurationJacobianSupport", ...
...
assert(numel(permutation)==contract.nx+contract.neq && ...
    ...
    "stageA3:solver:ReducedPermutation", ...
    "A3 reduced permutation is not bijective.");
