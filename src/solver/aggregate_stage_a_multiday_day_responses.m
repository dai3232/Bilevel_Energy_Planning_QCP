...
function aggregation = aggregate_stage_a_multiday_day_responses(responses,expectedDays)
%AGGREGATE_STAGE_A_MULTIDAY_DAY_RESPONSES Sort days before fixed-order sums.
...
assert(numel(responses)==7,"stageAMultiday:solver:DailyResponseCount", ...
    "formal multi-day aggregation requires exactly seven day responses.");
inputOrder = reshape([responses.day_id],1,[]);
assert(numel(unique(inputOrder))==7 && ...
    isequal(sort(inputOrder),expectedDays), ...
    "stageAMultiday:solver:DailyResponseDayIds", ...
...
aggregation = struct();
if isfield(sorted,"stage_id")
    stageIds = string({sorted.stage_id});
    assert(isscalar(unique(stageIds)), ...
        "stageAMultiday:solver:DailyResponseStageMismatch", ...
        "All daily responses must share one stage identity.");
    aggregation.stage_id = stageIds(1);
else
    aggregation.stage_id = "";
end
...
for d = 1:numel(responses)
    assert(isequal(size(responses(d).S),[14,14]) && ...
        ...
        "stageAMultiday:solver:DailyResponseDimension", ...
