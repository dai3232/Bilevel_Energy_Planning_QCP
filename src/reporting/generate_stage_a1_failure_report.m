function reportPath = generate_stage_a1_failure_report( ...
        runContext,exception,issues,acceptance)
%GENERATE_STAGE_A1_FAILURE_REPORT Preserve a minimal catch-path A1 report.
% The report is intentionally independent of numerical A1 artifacts. Its
...
reportPath = fullfile(reportsDirectory,'阶段A1_失败与首层定位报告.docx');
if isfile(reportPath) || isfolder(reportPath)
    error('stageA1:failureReport:ArtifactExists', ...
...
stagedPath = fullfile(stagingDirectory,'阶段A1_失败与首层定位报告.docx');
write_failure_docx(stagedPath,runId,bodyXml);
[valid,validation] = validate_docx_package(stagedPath);
if ~valid
    error('stageA1:failureReport:InvalidDocxPackage', ...
...
if ~moved
    error('stageA1:failureReport:PublishFailed', ...
...
function body = build_failure_body(runId,stageId,manifestStatus,gitCommit, ...
        exception,issues,acceptance)
parts = strings(0,1);
parts(end+1) = paragraph_xml('STAGE A1 · 失败证据备忘','Kicker');
parts(end+1) = paragraph_xml('阶段A1 失败与首层定位报告','Title');
...
parts(end+1) = paragraph_xml([ ...
    '本报告只证明 catch 路径已保存当时可获得的失败事实，不宣告 Stage A1 ' ...
    '通过，也不推断未运行的矩阵维数、方向误差或残差。未执行完整 IPM、并行、' ...
    '容量规划或经济性解释；正式 24 小时模型口径未改变。'],'Normal');
...
if strlength(textValue) == 0
    error('stageA1:failureReport:MissingManifestField', ...
...
if size(rows,2) ~= numel(headers) || numel(widths) ~= numel(headers) || ...
        sum(widths) ~= 9360
    error('stageA1:failureReport:TableGeometry', ...
...
if ~moved
    error('stageA1:failureReport:PackageWriteFailed', ...
...
xml = "<?xml version=""1.0"" encoding=""UTF-8"" standalone=""yes""?>"+ ...
    ...
    paragraph_xml("STAGE A1 · 失败闭环 | 运行 "+runId,'Header')+"</w:hdr>";
end
...
xml = ['<?xml version="1.0" encoding="UTF-8" standalone="yes"?>' ...
    ...
    'xmlns:dc="http://purl.org/dc/elements/1.1/"><dc:title>阶段A1 失败与首层定位报告</dc:title>' ...
    '<dc:creator>MATLAB stage_A1 reporting</dc:creator></cp:coreProperties>'];
end
...
if fileId < 0
    error('stageA1:failureReport:WriteFailed', ...
...
if written ~= numel(bytes)
    error('stageA1:failureReport:WriteFailed','Short write for %s.',pathValue);
end
status = fclose(fileId);
clear cleanup;
if status ~= 0
    error('stageA1:failureReport:WriteFailed','Could not close %s.',pathValue);
end
end
...
if isfile(pathValue)
    error('stageA1:failureReport:PathConflict', ...
        'A file exists where reports/ is required: %s',pathValue);
end
[created,message] = mkdir(pathValue);
if ~created
    error('stageA1:failureReport:CreateDirectoryFailed','%s',message);
end
end
