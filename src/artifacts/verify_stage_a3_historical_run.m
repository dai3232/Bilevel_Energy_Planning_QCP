...
actual=lower(string(compute_sha256_file(pathValue)));
...
for rowNumber=1:height(inventory)
    ...
    if exists
        actualHash=lower(string(compute_sha256_file(pathValue)));
    else
        actualHash="";
    end
...
for k=1:numel(sourceFiles)
    ...
    sameHash=sameBytes&&lower(string(compute_sha256_file(source)))== ...
        lower(string(compute_sha256_file(extracted)));
...
audit=struct("file_count",entryAudit.file_count, ...
    "sha256",lower(string(compute_sha256_file(zipPath))));
clear guard
end
