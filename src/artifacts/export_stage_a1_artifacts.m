...
save_mat_artifact(paths.linearization_mat,linearizationPayload);
save_mat_artifact(paths.full_kkt_mat,fullPayload);
save_mat_artifact(paths.recursive_blocks_mat,recursivePayload);
save_mat_artifact(paths.directions_mat,directionsPayload);
save_mat_artifact(paths.residuals_mat,residualPayload);
save_mat_artifact(paths.index_mapping_mat,"index",index);
save_mat_artifact(paths.configuration_mat,configurationPayload);
...
write_table_csv_17g(paths.fixed_zero_audit,fixedAudit);
...
for k = 1:n
    ...
    sha256(k) = compute_sha256_file(artifactPaths(k));
