...
%BUILD_STAGE_A3_LINEARIZATION Build the unique shared seven-day model view.
%
% Both complete-KKT audit and recursive reduction must consume this exact
% object.  It contains one global q, seven q_d copies, independent formal
...
if numel(state.xi) ~= nPrimal || numel(state.y) ~= nEquality || ...
    ...
    error("stageA3:linearization:StateDimension", ...
...
linearization.stage_id = "stage_A3";
...
if numel(maps.q_global) ~= 14
    error("stageA3:linearization:GlobalCapacityMap", ...
...
for dayPosition = 1:nDays
    ...
    if numel(qDay) ~= 14 || numel(binding) ~= 14
        error("stageA3:linearization:DailyCapacityMap", ...
...
for rowNumber = 1:nEquality
    ...
    elseif name == "soc_dynamics"
        ...
        if link.current_soc_global_index ~= currentSoc
            error("stageA3:linearization:CurrentSocMap", ...
        ...
        if isnan(link.predecessor_hour)
            if link.predecessor_soc_global_index ~= 0 || ...
                ...
                error("stageA3:linearization:InitialSocMap", ...
            ...
            if link.predecessor_soc_global_index ~= expectedPredecessor
                error("stageA3:linearization:SocPredecessorMap", ...
    ...
    elseif name == "terminal_soc"
        ...
        if ~link.terminal_equality || ...
            ...
            error("stageA3:linearization:TerminalSocMap", ...
        ...
        error("stageA3:linearization:UnknownEquality", ...
            "Unsupported Stage A3 equality %s.",name);
    end
end
end
...
for rowNumber = 1:nInequality
    ...
    if row.day == 0
        ...
        elseif contains(name,"upper")
            ...
            error("stageA3:linearization:GlobalBoundSide", ...
    ...
    if ~xor(isLower,isUpper)
        error("stageA3:linearization:HourlyBoundSide", ...
    ...
    elseif type == "storage" && variableName == "SOC"
        ...
        error("stageA3:linearization:UnknownUpperBound", ...
...
for dayPosition = 1:nDays
    ...
    for hourPosition = 1:nHours
        ...
        if height(hourLinks) ~= data.meta.nStorage
            error("stageA3:linearization:LayoutSocLinks", ...
...
if ~issparse(lin.H) || ~issparse(lin.A) || ~issparse(lin.G)
    error("stageA3:linearization:SparseContract", ...
...
if numel(lin.maps.y_duration) ~= 2 || ...
    ...
    error("stageA3:linearization:CanonicalSlices", ...
...
if nnz(lin.objective.gradient(lin.maps.q_day)) ~= 0 || ...
    ...
    error("stageA3:linearization:InvestmentObjectiveMultiplicity", ...
...
for dayPosition = 1:numel(config.days)
    ...
    if numel(balanceRow) ~= 1 || lin.A(balanceRow,pch) ~= -1 || ...
        ...
        error("stageA3:linearization:PowerBalanceSigns", ...
    ...
    for storage = 1:2
        ...
        if numel(firstRow) ~= 1 || ~isnan(firstLink.predecessor_hour) || ...
            ...
            error("stageA3:linearization:InitialSocBoundary", ...
    ...
    for hour = 2:24
        for storage = 1:2
            ...
            if numel(row) ~= 1 || ...
                ...
                error("stageA3:linearization:InternalSocCoefficient", ...
            ...
            if predecessor.day ~= day || predecessor.hour ~= hour-1
                error("stageA3:linearization:CrossDaySocCoupling", ...
                    "A3 contains a cross-day SOC coefficient.");
            end
        end
    end
    ...
    if numel(terminalRows) ~= 2 || any(equalities.hour(terminalRows) ~= 24)
        error("stageA3:linearization:TerminalSocRows", ...
...
if any(lin.fixed_zero_map.fixed_value ~= 0) || ...
    ...
    error("stageA3:linearization:FixedZeroMap", ...
...
if ~isscalar(position)
    error("stageA3:linearization:DayLookup", ...
...
if nnz(mask) ~= 1
    error("stageA3:linearization:SocLinkLookup", ...
...
if numel(sources) ~= 1
    error("stageA3:linearization:BoundarySource", ...
...
if all(isnan(values))
    predecessor = nan;
elseif all(isfinite(values)) && numel(unique(values)) == 1
    predecessor = values(1);
else
    error("stageA3:linearization:PredecessorHour", ...
...
elseif type == "storage" && startsWith(constraintName,"soc_")
    ...
    error("stageA3:linearization:ConstraintVariable", ...
...
if nnz(mask) ~= 1
    error("stageA3:linearization:VariableLookup", ...
...
if string(config.stage_id) ~= "stage_A3" || ...
        ...
        string(index.scope.stage_id) ~= "stage_A3"
    error("stageA3:linearization:StageIndexMismatch", ...
        "Configuration and index must both identify stage_A3.");
end
if ~isequal(reshape(index.scope.days,1,[]),config.days) || ...
    ...
    error("stageA3:linearization:ScopeMismatch", ...
        "A3 requires seven formal physical days, not an explicit test window.");
end
end
